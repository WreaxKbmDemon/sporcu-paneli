...
def render_analytics_tab(csv_file):
    ...
    try:
        ...
        if len(df) >= 2:
            ...
            if kilo_farki < 0:
                st.success(f"📉 İlk kayıttan itibaren jilet gibi **{abs(kilo_farki):.2f} kg** verdin. Yağ motoru çalışıyor!")
            elif kilo_farki > 0:
                st.warning(f"📈 İlk kayıttan itibaren **{kilo_farki:.2f} kg** aldın. Takibe devam!")
            else:
